...
public class HtmlFilterUtil {
    ...
    /**
     * 基本功能：过滤所有以"<"开头以">"结尾的标签，但不用正则表达式
     *
     * @param src   source string
     * @return      new string that replace all HTML tags
     */
    public static String filterAllHtmlTagNoRegex(String src) {
        if(src == null || src.length() < 3) return src;
        StringBuilder ret = new StringBuilder(src.length());
        int tmp = 0;
        while (tmp < src.length()) {
            int s = src.indexOf("<", tmp);
            if (s == tmp) {
                int e = src.indexOf(">", s);
                if (e < 0) {
                    ret.append(src.substring(s));
                    break;
                }
                tmp = e + 1;
            } else if (s > tmp) {
                ret.append(src.substring(tmp, s));
                tmp = s;
                //filter non-tag '<'
                if (s < src.length() - 1) {
                    char nextChar = src.charAt(s + 1);
                    if (nextChar == '<' || nextChar == ' '){ //不是tag的开始
                        ++tmp;
                        ret.append(src.charAt(s++));
                    }
                }
            } else {
                ret.append(src.substring(tmp));
                break;
            }
        }
