...
public class PaymentCommonUtils {
	...
	public static <T> T jsonStrToObject(String jsonStr, Class<T> clazz) {
		...
		try {
			...
			logger.error(String.format("the json parse error: %s  %s", jsonStr,
					e.getMessage()));
		}
		return ret;
	}
	 ...
	 * @return
	 */
	public static Map<Long, BigDecimal> findFeeCoupon(BigDecimal realCost,
	...
	/*
	 * private String encoder(String cvalue) { try { cvalue =
	 * URLEncoder.encode(cvalue, "utf-8"); } catch (UnsupportedEncodingException
	 * e) { cvalue = null; } return cvalue; } private String decoder(String
	 * cvalue) { try { cvalue = URLDecoder.decode(cvalue, "utf-8"); } catch
	 * (UnsupportedEncodingException e) { cvalue = null; } return cvalue; }
	 */
}
